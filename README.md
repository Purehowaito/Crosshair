...
**基于C++开发的Windows桌面应用程序，通过创建透明的顶层窗口在屏幕上绘制准心，以帮助您在游戏中进行视觉定位；程序不涉及进程注入、内存修改或系统驱动安装，因此游戏在以独占全屏下运行时可能会被覆盖；使用时您需遵循游戏运营方的用户协议及反作弊策略的允许下运行。**

**C++で開発されたWindowsデスクトップアプリケーションで、透明な最前面ウィンドウを作成して画面上に照準を描画し、ゲーム内での視覚的な照準合わせを支援します。このプログラムはプロセス注入、メモリ改ざん、システムドライバのインストールを一切行わないため、ゲームが排他的フルスクリーンモードで実行されている場合に照準が覆われてしまう可能性があります。ご使用にあたっては、ゲーム運営元のユーザー規約およびアンチチートポリシーで許可されている場合に限り実行してください。**
